...
import { ChevronRight, Users, Clapperboard, Film, Mic, Star, Search } from 'lucide-react';
import MobileNav from '../components/MobileNav';

const gold = '#c9a96e';

export const metadata: Metadata = {
  title: 'Miami Casting Agency — Find Models, Actresses & Voiceover Talent | Influence',
  description: 'Influence Models is a Miami casting agency connecting productions with professional models, actresses, and voiceover talent. Casting for commercials, TV, film, and brand campaigns in South Florida.',
  openGraph: {
    title: 'Miami Casting Agency — Models, Actresses & Voiceover Talent | Influence',
    description: 'Source professional talent for commercials, TV, film, and brand productions.',
    url: 'https://influencemodels.agency/casting-agency-miami',
  },
};

export default function CastingAgencyMiamiPage() {
  return (
    <div className="min-h-screen bg-[#080808]">
      <nav className="border-b border-white/[0.06] bg-[#080808]/95 backdrop-blur-xl fixed w-full z-50">
        <div className="flex items-center justify-between h-16 md:h-20 px-6 md:px-14">
          ...
          <div className="flex items-center gap-10">
            ...
            <Link href="/contact" className="text-[11px] md:text-[12px] tracking-widest uppercase px-5 py-3 md:px-8 md:py-4 transition-all duration-300 hover:opacity-80 font-bold" style={{ backgroundColor: gold, color: '#000' }}>Cast a Project</Link>
          </div>
        </div>
      </nav>

      <section className="relative min-h-[85vh] flex items-end overflow-hidden">
        <img src="/images/Des/des-21.jpg" alt="Miami casting agency" className="absolute inset-0 w-full h-full object-cover object-top" />
        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-black/20" />
        <div className="absolute inset-0 bg-gradient-to-r from-black/70 to-transparent" />
        <div className="relative z-10 px-8 md:px-16 pb-16 md:pb-24 max-w-4xl">
          <p className="text-[10px] font-bold tracking-[0.4em] uppercase mb-6" style={{ color: gold }}>South Florida Casting Partner</p>
          ...
          <p className="text-white/60 text-lg md:text-xl max-w-xl leading-relaxed mb-10">
            We source and coordinate talent for commercials, TV, film, brand campaigns, and creative productions. Our roster includes models, actresses, and voiceover artists with Netflix, Audible, and major commercial experience.
          </p>
          <div className="flex flex-col sm:flex-row gap-4">
            <Link href="/contact" className="inline-flex items-center justify-center gap-3 px-10 py-5 text-[13px] font-bold tracking-widest uppercase hover:opacity-85 transition-all" style={{ backgroundColor: gold, color: '#000' }}>
              Cast a Project <ChevronRight className="h-4 w-4" />
            </Link>
      ...
      <section className="py-20 md:py-28 px-8 md:px-16 border-t border-white/[0.06]">
        <p className="text-[10px] font-bold tracking-[0.4em] uppercase mb-6" style={{ color: gold }}>How We Work</p>
        ...
        <p className="text-white/40 text-lg max-w-2xl mb-14 leading-relaxed">
          From casting calls to final bookings, we handle the heavy lifting — vetting talent, coordinating schedules, and making sure your production has the right faces and voices.
        </p>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl">
          {[
            { icon: Search, title: 'Casting Calls', desc: 'We organize and promote casting calls for your project or brand campaign.' },
            { icon: Users, title: 'Roster Curation', desc: 'Hand-picked models, actresses, and voiceover talent from our verified marketplace.' },
            { icon: Film, title: 'TV & Film Casting', desc: 'Source talent for scripted roles, background, featured extras, and leads.' },
            { icon: Clapperboard, title: 'Commercial Casting', desc: 'Find talent with on-camera speaking ability and commercial experience.' },
            { icon: Mic, title: 'Voiceover Casting', desc: 'Match your brand with the right voice — narration, ads, audiobooks, and more.' },
            { icon: Star, title: 'Verified Credits', desc: 'Talent with Netflix, Audible, Peacock, and major brand campaigns on their résumé.' },
      ...
      <section className="py-20 md:py-32 px-8 md:px-16 bg-white border-t border-black/10">
        <div className="max-w-3xl">
          <p className="text-black/40 text-[10px] tracking-[0.5em] uppercase mb-8 font-semibold">Start Casting</p>
          <h2 className="font-display font-bold italic text-black leading-[0.9] mb-6" style={{ fontSize: 'clamp(40px, 7vw, 80px)' }}>
            Your production.{'\n'}Our talent.
          </h2>
          <p className="text-black/50 text-lg mb-10 max-w-lg">Tell us what you need and we’ll deliver a curated list of available talent for your project. South Florida and remote options.</p>
          <Link href="/contact" className="inline-flex items-center justify-center gap-3 px-12 py-6 text-[14px] font-bold tracking-widest uppercase hover:opacity-85 transition-all" style={{ backgroundColor: gold, color: '#000' }}>
            Request a Casting <ChevronRight className="h-4 w-4" />
          </Link>
        </div>
      </section>
